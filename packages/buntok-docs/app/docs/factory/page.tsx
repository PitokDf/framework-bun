...
export const metadata = {
  title: "Factory",
  description: "Type-safe data factories for generating test and seed data with @faker-js/faker.",
};

export default function FactoryPage() {
  return (
    <div>
      <Heading level={1} className="text-4xl font-bold mt-8 mb-4 text-text-primary">Factory</Heading>
      <p className="my-3 text-text-secondary leading-relaxed">
        Type-safe data factories for generating test and seed data. Factories provide a clean, reusable way to create test data with sensible defaults and easy overrides.
      </p>
      ...
      <p className="my-3 text-text-secondary leading-relaxed">
        <code>@faker-js/faker</code> is an optional peer dependency for generating realistic fake data:
      </p>
...
// → { id: 5678, name: "Jane Smith", email: "jane@example.com", role: "admin" }`}
      ...
      <p className="my-3 text-text-secondary leading-relaxed">
        Use <code>build()</code> for synchronous generation without the <code>afterCreate</code> hook:
      </p>
      <CodeBlock
...
const admin = await UserFactory.create({ role: "admin" });  // role: "admin"`}
      ...
      <p className="my-3 text-text-secondary leading-relaxed">
        Run custom logic after each item is created (useful for database inserts, audit logs, etc.):
      </p>
      <CodeBlock
        code={`const UserFactory = Factory.define<User>(() => ({
  id: faker.number.int({ max: 10000 }),
...
// → ["api", "typescript"]`}
      ...
      <p className="my-3 text-text-secondary leading-relaxed">
        Generate seeders that use factories with the <code>--factory</code> flag:
      </p>
...
export async function seedUser() {
  ...
  console.log("✓ User seeded successfully (100 records)");
      ...
      <div className="overflow-x-auto my-4">
        <table className="min-w-full border border-border-primary text-sm">
          ...
          <tbody>
            ...
            <tr><td className="border border-border-primary px-4 py-2 text-text-secondary"><code>.build(overrides?)</code></td><td className="border border-border-primary px-4 py-2 text-text-secondary">Generate one item synchronously (no afterCreate)</td></tr>
            ...
            <tr><td className="border border-border-primary px-4 py-2 text-text-secondary"><code>.withDefaults(defaults)</code></td><td className="border border-border-primary px-4 py-2 text-text-secondary">Set default overrides for all created items</td></tr>
            <tr><td className="border border-border-primary px-4 py-2 text-text-secondary"><code>.afterCreate(callback)</code></td><td className="border border-border-primary px-4 py-2 text-text-secondary">Register post-creation hook (async supported)</td></tr>
      ...
      <Callout type="info">
        Factories are designed for test and seed data generation. For production data, use your ORM's create methods directly.
      </Callout>
    </div>
  );
}
