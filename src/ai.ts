  ...
  const readable = new ReadableStream({
    async start(controller) {
      try {
        ...
        controller.enqueue(new TextEncoder().encode(`e:${JSON.stringify({ message: e.message })}\n`));
      } finally {
        controller.close();
      }
    }
...
export class AICache {
  ...
  private hashMessages(messages: any[]): string {
    const relevantMessages = messages.filter(m => m.role === 'user' || m.role === 'assistant').slice(-3);
   ...
   */
  async set(messages: any[], responseText: string, ttlSeconds: number = 3600): Promise<void> {
    const key = this.hashMessages(messages);
