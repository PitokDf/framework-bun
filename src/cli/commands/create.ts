...
async function ensureDirectories(entityName: string) {
...
export async function createCommand(entityName: string, args: string[]) {
  ...
  const generateFile = async (path: string, contentGenerator: () => string, type: string) => {
    if (!existsSync(path)) {
  ...
  if (options.schema) {
    tasks.push(generateFile(join("src/db/schemas", `${entityName}.ts`), () => generateSchema(entityName), "Schema"));
  }

  if (options.repo) {
    tasks.push(generateFile(join("src/repositories", `${entityName}.repository.ts`), () => generateRepository(entityName, pascalName, options.schema), "Repository"));
  }

  if (options.service) {
    tasks.push(generateFile(join("src/services", `${entityName}.service.ts`), () => generateService(entityName, pascalName, options.repo), "Service"));
  }

  if (options.controller) {
    tasks.push(generateFile(join("src/controllers", `${entityName}.controller.ts`), () => generateController(entityName, pascalName, options.service), "Controller"));
  }
  ...
  if (generatedFiles.length > 0) {
    const biomeProc = Bun.spawnSync(["bunx", "biome", "format", "--write", ...generatedFiles], {
      stdio: ["ignore", "ignore", "ignore"]
    });
    if (biomeProc.exitCode === 0) {
      console.log("\x1b[90m✨ Auto-formatted generated files with Biome\x1b[0m\n");
    }
  }
